...
public class FFTAuxiliary {
    ...
    static <FieldT extends AbstractFieldElementExpanded<FieldT>> void serialRadix2FFT(final List<FieldT> input,final FieldT omega) {

    final int n = input.size();//# Get the length of the input
    ...
    //System.out.println("original omega:" + Hex.encodeHexString(omega.toBigInteger().toByteArray()));

    int m = 1; // invariant: m = 2^{s-1}
    for (int s = 1; s <= logn; ++s) {
        // w_m is 2^s-th root of unity now.  wn is principle complex nth root of unity.
